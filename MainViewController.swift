...
class MainViewController: UIViewController {
    ...
    @IBAction func cancelAnnotation(_ sender: Any) {
        self.annotationView.isHidden = true
        self.annotationText.resignFirstResponder()
    }
 ...
 @IBAction func saveAnnotation(_ sender: Any) {
    ...
    annotation.tag = annotationCount + 1
    
    
    
    let ref = Database.database().reference().child("paintings").child(annotation.painting).child(annotation.name).child(annotation.mode).child(annotation.key)
    
    ref.setValue(["data": self.annotationText.text, "tag": annotation.tag, "x": annotation.x, "y": annotation.y])
    self.annotationView.isHidden = true
    self.annotationText.resignFirstResponder()
    annotationCount += 1
    self.numberOfAnnotationsLabel.text = String(self.annotationCount)
    self.annotationText.text = "Describe the painting"
    }
    
    
    
    @IBAction func showAnnotations(_ sender: Any) {
        let myVC = storyboard?.instantiateViewController(withIdentifier: "NormalModeViewController") as! NormalModeViewController
        
        myVC.namePassed = self.namePassed
        myVC.paintingPassed = self.paintingPassed
        myVC.mode = "AR"
        navigationController?.pushViewController(myVC, animated: true)
    }
    
    
    @IBOutlet weak var numberOfAnnotationsLabel: UILabel!
	...
	override func viewDidLoad() {
        ...
        ref.observe(.value, with: {
            ...
            self.annotationCount = Int(snapshot.childrenCount)
            self.numberOfAnnotationsLabel.text = String(self.annotationCount)
        })
    ...
    @IBAction func insertEaselButton(_ sender: Any) {
        let objectViewController = VirtualObjectSelectionViewController(size: CGSize(width: 0, height: 0))
        var object = objectViewController.getObject(index: 5)
        print(paintingPassed)
        switch paintingPassed {
        case "Painting 1":
             object = objectViewController.getObject(index: 5)
        case "Painting 2":
             object = objectViewController.getObject(index: 6)
        case "Painting 3":
             object = objectViewController.getObject(index: 7)
        default:
             object = objectViewController.getObject(index: 5)
        }
        print(object.modelName)
        virtualObjectSelectionViewController(objectViewController, object: object)
...
//			self.loadVirtualObject()
	...
	@IBAction func takeSnapShot() {
        ...
        let planeDimension: CGFloat = 0.5
        let path = Bundle.main.path(forResource: "painting", ofType: "png", inDirectory: "Models.scnassets")!
